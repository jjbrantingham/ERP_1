...
/// <summary>
/// Handler for Balance Sheet report
/// </summary>
public class GetBalanceSheetQueryHandler : IRequestHandler<GetBalanceSheetQuery, BalanceSheetDto>
{
    ...
    public async Task<BalanceSheetDto> Handle(GetBalanceSheetQuery request, CancellationToken cancellationToken)
    {
        ...
        // Assets (1000-1999)
        report.Assets = journalLines
            .Where(l => l.Account.AccountNumber.StartsWith("1"))
            .GroupBy(l => new { l.Account.AccountNumber, l.Account.Name, l.Account.AccountType })
            .Select(g => new AssetLineDto
            {
                AccountNumber = g.Key.AccountNumber,
                AccountName = g.Key.Name,
                Category = g.Key.AccountType.ToString(),
                Amount = g.Sum(l => l.DebitAmount - l.CreditAmount) // Assets are debit balance
            })
            .Where(l => l.Amount != 0)
            .OrderBy(l => l.AccountNumber)
            .ToList();
        ...
        report.Liabilities = journalLines
            .Where(l => l.Account.AccountNumber.StartsWith("2"))
            .GroupBy(l => new { l.Account.AccountNumber, l.Account.Name, l.Account.AccountType })
            .Select(g => new LiabilityLineDto
            {
                AccountNumber = g.Key.AccountNumber,
                AccountName = g.Key.Name,
                Category = g.Key.AccountType.ToString(),
                Amount = g.Sum(l => l.CreditAmount - l.DebitAmount) // Liabilities are credit balance
            })
            .Where(l => l.Amount != 0)
            .OrderBy(l => l.AccountNumber)
            .ToList();
        ...
        // Equity (3000-3999)
        report.Equity = journalLines
            .Where(l => l.Account.AccountNumber.StartsWith("3"))
            .GroupBy(l => new { l.Account.AccountNumber, l.Account.Name })
            .Select(g => new EquityLineDto
            {
                AccountNumber = g.Key.AccountNumber,
                AccountName = g.Key.Name,
                Amount = g.Sum(l => l.CreditAmount - l.DebitAmount) // Equity is credit balance
            })
            .Where(l => l.Amount != 0)
            .OrderBy(l => l.AccountNumber)
            .ToList();
